...
public class ViewPastServlet extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        //int custID = 1; // Replace with session.getAttribute("custID") when using sessions
        ...
        try {
            ...
            PreparedStatement stmt = conn.prepareStatement(
                    "SELECT FT.ticketID, FT.purchasedate, F.airline_id, F.flight_number, FTF.departure_date, F.depart_airportID, F.arrival_airportID " +
                    ...
                    "ORDER BY FTF.departure_date DESC");
            stmt.setInt(1, custID);
            ResultSet rs = stmt.executeQuery();

            request.setAttribute("results", rs);
            RequestDispatcher rd = request.getRequestDispatcher("viewPast.jsp");
            rd.forward(request, response);
        } catch (Exception e) {
            e.printStackTrace();
            response.getWriter().println("Error retrieving past flights: " + e.getMessage());
        }
    }
}